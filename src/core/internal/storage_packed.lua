...
---@param prefix string
function storage.validatePackedBits(node, prefix)
    local seenAliases = {}
    local occupiedBits = {}
...
---@return any
function storage.DecodePackedChild(node, packedValue)
    local rawValue = storage.readPackedBits(packedValue, node.offset, node.width)
...
function storage.getPackedAliases(node)
